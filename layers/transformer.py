...
from layers.attention import (
    MultiScaleMultiHeadNA1D,
    MultiScaleMultiHeadNA2D,
    MultiScaleMultiHeadNA3D,
)
...
class MSNATTransformer(ABC, Module, Generic[ConvType, MultiScaleMultiHeadNAType]):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.final_conv = nn.Sequential(
            self.conv_type(
                kernel_size=1, in_channels=out_channels, out_channels=out_channels * 4
            ),
            nn.GELU(),
            self.conv_type(
                kernel_size=1, in_channels=out_channels * 4, out_channels=out_channels
            ),
        )
    ...
    def _set_rescale(self):
        if self.scale_factor != 1:
            self.rescale = lambda x: interpolate(
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = self.residual_downsample(x) + self.multi_head_attention(
            self.dropout(self.layernorm1(x))
        )
        x = x + self.final_conv(self.dropout(self.layernorm2(x)))
        return x


class MSNATTransformer1D(MSNATTransformer[Conv1d, MultiScaleMultiHeadNA1D]):
    multi_head_attention_type = MultiScaleMultiHeadNA1D
    conv_type = Conv1d
    norm_type = LayerNorm


class MSNATTransformer2D(MSNATTransformer[Conv2d, MultiScaleMultiHeadNA2D]):
    multi_head_attention_type = MultiScaleMultiHeadNA2D
    conv_type = Conv2d
    norm_type = LayerNorm2d


class MSNATTransformer3D(MSNATTransformer[Conv3d, MultiScaleMultiHeadNA3D]):
    multi_head_attention_type = MultiScaleMultiHeadNA3D
    conv_type = Conv3d
...
class GlobalAttentionTransformer(nn.Module):
    ...
    def initialize_tokens(
        self,
        batch_size: int,
        context_token: Optional[torch.Tensor],
    ...
    def forward(
        ...
        # Use single qkv projection and split
        ...
        attn_output = flex_attention(
            query=queries, key=keys, value=values, block_mask=block_mask
        )
