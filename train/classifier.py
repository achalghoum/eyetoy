    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (inputs, labels) in enumerate(train_loader):
            ...
            print(f"Epoch {epoch+1}, Batch {batch_idx}, Loss: {loss.item():.4f}, "
                    f"Accuracy: {100.*train_correct/train_total:.2f}%, Gradient norm: {total_norm:.4f}")
...
if __name__ == "__main__":
    ...
    train_loader = DataLoader(caltech_256_train, batch_size=32, shuffle=True, num_workers=4)
    val_loader = DataLoader(caltech_256_val, batch_size=32, shuffle=False, num_workers=4)
    ...
    train_encoder_classifier(model, train_loader, val_loader, num_epochs=10, 
                             learning_rate=0.0005, device=device, weight_decay=weight_decay)
